-- Créer une feuille d'émargement démo avec 10 étudiants
DO $$
DECLARE
...
BEGIN
    ...
    -- Marquer les 3 derniers étudiants comme absents (sans signature)
    FOR i IN 8..10 LOOP
        INSERT INTO attendance_signatures (
            attendance_sheet_id, user_id, user_type, present, signed_at, 
            signature_data, absence_reason, absence_reason_type
        ) VALUES (
            demo_attendance_sheet_id,
            student_ids[i],
            'student',
            ...
            NULL,
            CASE 
                WHEN i = 8 THEN 'Maladie'
                WHEN i = 9 THEN 'Rendez-vous médical'
                WHEN i = 10 THEN 'Transport'
            END,
            CASE 
                WHEN i = 8 THEN 'justified'
                WHEN i = 9 THEN 'justified'
                WHEN i = 10 THEN 'unjustified'
            END
        );
    END LOOP;
