...
import {
  X, Download, ExternalLink, Loader2, FileText, AlertCircle,
...
/**
 * Unified file viewer (replaces the previous heavy `ProductionFileViewer`).
 *
 * Adopts the exact same chrome as `PdfViewerModal` so every file viewing across
 * the application looks identical to the formation référentiel viewer.
 *
 * Body adapts to file kind: pdf/image/video/audio/office/text/archive.
 */
const ProductionFileViewer: React.FC<ProductionFileViewerProps> = ({
  ...
  // Resolve signed URLs (private storage) when needed.
  ...
  // Reset state on open / URL change
  useEffect(() => {
    if (isOpen) {
  ...
  const renderBody = () => {
    ...
    if (kind === 'pdf') {
      ...
      if (iframeError) {
        return <FallbackUnsupported url={url} onDownload={handleDownload} />;
      }
      return (
        <>
          {!iframeLoaded && (
    ...
    if (kind === 'audio') {
      return <AudioViewer fileUrl={url} fileName={fileName} />;
    }

    if (kind === 'text') {
      return <TextViewer fileUrl={url} fileName={fileName} />;
    }

    if (kind === 'archive') {
      return <ArchiveViewer fileUrl={url} fileName={fileName} />;
    }

    if (kind === 'office') {
      // Office Online viewer requires a public URL accessible by Microsoft. If the file is
      // already a public URL, embed via Office Online; otherwise show a graceful fallback.
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(o) => !o && onClose()}>
      ...
      >
        ...
        {/* Header — identical to PdfViewerModal for full UX consistency */}
        <div className="flex items-center justify-between gap-2 px-3 sm:px-5 py-3 border-b border-border bg-card/95 backdrop-blur-sm shrink-0">
          <div className="flex items-center gap-2 min-w-0 flex-1">
            ...
            <div className="min-w-0">
              <h3 className="text-sm sm:text-base font-semibold text-foreground truncate" data-testid="file-viewer-title">
                {fileName}
              </h3>
              {ext && (
                <p className="text-[11px] text-muted-foreground truncate hidden sm:block uppercase">{ext}</p>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            <Button
              ...
              data-testid="file-viewer-download-mobile"
              title="Télécharger"
            >
              <Download className="h-4 w-4" />
            </Button>
            <Button
              ...
              className="h-9 w-9"
              data-testid="file-viewer-newtab"
...
const FallbackUnsupported: React.FC<{ url: string; onDownload: () => void; message?: string }> = ({
  url,
  onDownload,
  message = "Impossible d'afficher le fichier dans le navigateur",
}) => (
  <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 px-6 text-center">
