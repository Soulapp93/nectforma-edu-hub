...
/**
 * Resizable + draggable PDF viewer.
 * Includes a native presentation mode (Fullscreen API) — clean, toolbar-free,
 * exactly like Canva / PowerPoint slideshow. Press Esc to exit.
 */
const PdfViewerModal: React.FC<Props> = ({ open, onClose, url, title = 'Référentiel de formation', filename }) => {
  ...
  // Track native fullscreen state changes (Esc / F11 / app exit)
  useEffect(() => {
    const handler = () => {
      const inFs = !!document.fullscreenElement;
      setIsPresenting(inFs);
    };
  ...
  /**
   * Open a clean, toolbar-free presentation overlay and put it in OS fullscreen
   * via the native Fullscreen API. The overlay itself is rendered (below) only
   * when isPresenting=true; we trigger requestFullscreen() in a useEffect once
   * the ref is attached.
   */
  const enterPresentation = () => setIsPresenting(true);
  ...
  // When presentation overlay appears, request fullscreen on it.
  useEffect(() => {
  ...
  // Clean URL for presentation mode: NO toolbar, NO scrollbar, fit page to screen
  const presentationUrl = `${url}#toolbar=0&navpanes=0&scrollbar=0&statusbar=0&view=Fit`;
  ...
  return (
    <>
          ...
          >
            ...
            <div className="flex items-center gap-1 shrink-0" data-no-drag>
              ...
              <Button
                ...
                title="Mode diaporama (plein écran total)"
              >
              ...
              <Button
                ...
                title={isMaximized ? 'Restaurer' : 'Plein écran (fenêtre)'}
              >
      ...
      {/* PRESENTATION OVERLAY — true fullscreen, clean, no toolbar */}
      {isPresenting && createPortal(
        <div
          ...
          onClick={(e) => {
            // Only the floating "Quitter" button handles exit; iframe area is non-interactive for parent
            e.stopPropagation();
          }}
        >
          <iframe
          ...
          {/* Floating Quitter button (top-right) */}
          <button
            type="button"
            onClick={exitPresentation}
            className="fixed top-4 right-4 z-[310] inline-flex items-center gap-2 px-4 py-2 rounded-full bg-black/70 hover:bg-black/85 text-white text-sm font-medium backdrop-blur-md border border-white/20 shadow-2xl transition-all"
